...
import splitgui from './splitsgui';
import { data } from "./splitsdata";
...
const example = {
    a: `${returnColor(splitgui.supplyColor)}§lSupplies: &f0.00s`,
    b: `${returnColor(splitgui.buildColor)}§lBuild: &f0.00s`,
    c: `${returnColor(splitgui.eatenColor)}§lEaten: &f0.00s`,
    d: `${returnColor(splitgui.stunColor)}§lStun: &f0.00s`,
    e: `${returnColor(splitgui.DPSColor)}§lDPS: &f0.00s`,
    f: `${returnColor(splitgui.skipColor)}§lSkip: &f0.00s`,
    g: `${returnColor(splitgui.P4Color)}§lKuudra: &f0.00s`,
    h: `${returnColor(splitgui.overallColor)}§lOverall: &f0.00s`
};
...
register("renderOverlay", () => {
    if (splitgui.SplitsGUI.isOpen()) {
        ...
        for (let k in example) {
            Renderer.drawStringWithShadow(example[k], pos[0] + xOffset, pos[1] + 5 + (t * yOffset * scale));
            t++;
        }
...
function KuudraDown() {
    ...
    phase = 8;
        sleep(500, () => {
            ...
            ChatLib.chat(`&4Overall: &f${formatTimeMs(overall)}`);
            ChatLib.chat("");
            ChatLib.chat(`&d&l[IQ] Misc. Info`);
...
function updateTime() {
    ...
    overall = parseFloat(parseFloat(times[0] || 0.00) + parseFloat(times[1] || 0.00) + parseFloat(times[2] || 0.00) + parseFloat(times[3] || 0.00) + parseFloat(times[4] || 0.00) + parseFloat(times[5] || 0.00) + parseFloat(times[6] || 0.00) + lagCompensation).toFixed(2)
    
    if (phase > 0) {
        lines = {
            a: new Text(`${colors[0]}§lSupplies: ${getColorCode(times[0], 0)}${formatTimeMs(times[0])}`, pos[0] + 5, pos[1] + 5 * scale),
            b: new Text(`${colors[1]}§lBuild: ${getColorCode(times[1], 1)}${formatTimeMs(times[1])}`, pos[0] + 5, pos[1] + 15 * scale),
            c: new Text(`${colors[2]}§lEaten: ${getColorCode(times[2], 2)}${formatTimeMs(times[2])}`, pos[0] + 5, pos[1] + 25 * scale),
            d: new Text(`${colors[3]}§lStun: ${getColorCode(times[3], 3)}${formatTimeMs(times[3])}`, pos[0] + 5, pos[1] + 35 * scale),
            e: new Text(`${colors[4]}§lDPS: ${getColorCode(times[4], 4)}${formatTimeMs(times[4])}`, pos[0] + 5, pos[1] + 45 * scale),
            f: new Text(`${colors[5]}§lSkip: ${getColorCode(times[5], 5)}${formatTimeMs(times[5])}`, pos[0] + 5, pos[1] + 55 * scale),
            g: new Text(`${colors[6]}§lKuudra: ${getColorCode(times[6], 6)}${formatTimeMs(times[6])}`, pos[0] + 5, pos[1] + 65 * scale),
            h: new Text(`${colors[7]}§lOverall: &f${formatTimeMs(overall)}`, pos[0] + 5, pos[1] + 75 * scale)
        };
    }
...
function render() {
    ...
    for (let w in lines) {
        lines[w].setScale(scale).setShadow(true).draw();
    }
...
// função para formatar os freshers, utilizado no splits do chat no final da kill
...
// adiciona quem freshou no array de freshers
...
// anunciar quando vc fresha no chat
...
// anunciar tempo de supply no chat
